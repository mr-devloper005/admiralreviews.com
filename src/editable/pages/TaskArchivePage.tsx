...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': '#101113', '--archive-text': '#f8f8f3', '--archive-surface': '#151619', '--archive-accent': '#3f7cff', '--editable-border': 'rgba(255,255,255,0.12)', '--editable-container': '1120px' } as CSSProperties
  const categoryLabel = category === 'all' ? 'All categories' : CATEGORY_OPTIONS.find((item) => item.slug === category)?.name || category

  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.75fr] lg:px-8 lg:py-16">
          <div className="rounded-lg border border-[var(--editable-border)] bg-[var(--archive-surface)] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.18)] sm:p-10">
            <div className="inline-flex items-center gap-2 rounded-md border border-[var(--editable-border)] bg-white/5 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--archive-accent)]"><Icon className="h-4 w-4" /> {label}</div>
            <h1 className="mt-5 max-w-4xl text-5xl font-black uppercase leading-[0.92] tracking-tight sm:text-7xl">{voice?.headline || `Browse ${label}`}</h1>
            <p className="mt-6 max-w-2xl text-base font-bold leading-8 text-white/62">{voice?.description || SITE_CONFIG.description}</p>
            <div className="mt-6 rounded-md border border-[var(--editable-border)] bg-white/5 p-4 text-sm font-bold leading-7 text-white/68">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="rounded-md bg-[#3f64f4] px-5 py-3 text-sm font-black text-white">Browse all</Link>
              <Link href="/search" className="rounded-md border border-[var(--editable-border)] px-5 py-3 text-sm font-black">Search articles</Link>
            </div>
          </div>

          <form action={basePath} className="self-end rounded-lg border border-[var(--editable-border)] bg-[#151619] p-5 shadow-sm backdrop-blur">
            ...
            <select name="category" defaultValue={category} className="mt-4 h-12 w-full rounded-md border border-[var(--editable-border)] bg-white px-4 text-sm font-bold text-[#101113] outline-none">
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className="mt-3 h-12 w-full rounded-md bg-[#3f64f4] text-sm font-black text-white">Apply</button>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            ...
            <div className="rounded-lg border border-dashed border-[var(--editable-border)] bg-white/5 p-10 text-center">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-md border border-[var(--editable-border)] bg-white/5 px-5 py-3 text-sm font-black">Previous</Link> : null}
            <span className="rounded-md bg-[#3f64f4] px-5 py-3 text-sm font-black text-white">Page {page} of {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-md border border-[var(--editable-border)] bg-white/5 px-5 py-3 text-sm font-black">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden rounded-lg border border-[var(--editable-border)] bg-[#151619] transition hover:-translate-y-1 hover:bg-[#191b22]">
      <div className="relative aspect-[4/3] overflow-hidden bg-black/5">
        <img src={image} alt="" className="h-full w-full object-cover grayscale transition duration-500 group-hover:scale-105 group-hover:grayscale-0" />
        <span className="absolute left-4 top-4 rounded-md bg-[#3f64f4] px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-white">{category}</span>
      </div>
      <div className="p-5">
        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--archive-accent)]">Story {String(index + 1).padStart(2, '0')}</p>
        <h2 className="mt-2 text-xl font-black leading-tight tracking-tight text-white">{post.title}</h2>
        <p className="mt-3 line-clamp-3 text-sm leading-6 text-white/56">{getSummary(post)}</p>
      </div>
    </Link>
  )
}
